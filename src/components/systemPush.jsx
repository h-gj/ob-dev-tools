...
class SystemPush extends React.Component {
  ...
  handleEnv(event) {
    ...
    this.setState({env: env, baseUrl: baseUrl, wxPushEnabled: null, smsPushEnabled: null});
  }
  ...
  handleListPushableUsers(event) {
    ...
    const url = `${
      this.state.baseUrl
    }/api/admin/system/push/qualified-users`;
  ...
  handleQualifyUsers(event) {
    ...
    const url = `${
      this.state.baseUrl
    }/api/admin/system/push/users/batch-make-qualified`;
    console.log("SNO:", SNO);
  ...
  render() {
    return (<div className="App">
      ...
      {
      this.state.wxPushEnabled !== null ? <div className="beforePushInfo">
        <div>
          <Badge variant="secondary">微信开关</Badge>&nbsp;
          <Badge variant="primary" id="wx-push-enabled">
            {
            this.state.wxPushEnabled === null ? "" : this.state.wxPushEnabled === true ? "已开启" : "未开启"
          } </Badge>
        </div>
        ...
        <div>
          ...
          <Badge variant="warning" id="sms-push-enabled">
            {
            this.state.wxPushEnabled === null ? "" : this.state.smsPushEnabled === true ? "已开启" : "未开启"
          } </Badge>
        </div>
      ...
      <div>
        <h2>赋予资格</h2>
        <span>
          不是会员=>会员，最近7天有学习=>未学习，推送次数等于3=>2，请确保
          <i>
            <b>已绑定了手机号或服务号</b>
          </i>
        </span>
